require(["d3/3"], function(t) {
    function e(t) {
        for (var a in e.defaults) a in t || (t[a] = e.defaults[a]);
        for (var r = t.investmentReturnRate * (1 - t.capitalGainsRate), n = 1 - t.marginalTaxRate, i = 1 + t.inflationRate, o = Math.pow(1 + r, t.years) - 1, s = 1 + t.buyGrowthRate, u = t.buyPrice * t.buyDownPaymentRate, l = t.buyPrice * t.buyPurchaseClosingRate, c = u + l, p = t.buyInterestRate / 12, d = t.buyPrice * (1 - t.buyDownPaymentRate), h = Math.round(12 * t.buyLoanYears), g = d * (p ? p / (1 - Math.pow(1 + p, -h)) : 1 / h), m = 0, f = 0, y = 0, b = 12 * t.buyCommonChargePerMonth * (1 - t.marginalTaxRate * t.buyCommonChargeDeductionRate), v = 0, x = t.buyPropertyTaxRate * n, M = 0, R = 12 * t.buyUtilitiesCostPerMonth, C = 0, P = t.buyPrice * t.buyMaintenanceCostRate, k = 0, w = 0, F = c * o, S = 0, T = 1; T <= t.years; ++T) {
            var A = Math.pow(i, T - 1),
                N = t.buyPrice * Math.pow(s, T);
            if (S += (S + w) * r, h > 0) {
                var I = Math.min(h, 12),
                    q = Math.pow(1 + p, I),
                    D = d - (d = d * q - (p ? (q - 1) / p : 12) * g);
                f += (I * g - D) * n, m += D, h -= I
            }
            y += b * A, v += N * x, M += R * A, C += P * A, k += N * t.buyInsuranceRate, w = m + f + y + v + M + C + k
        }
        var _ = N * t.buySellClosingRate,
        ...
        return {
            ...
            buyPropertyTaxes: v,
            buyUtilitiesCost: M,
            buyMaintenanceCost: C,
            buyInsuranceCost: k,
            buyPurchaseOpportunityCost: F,
            buyYearlyOpportunityCost: S,
            buySellProceeds: -N,
            buySellClosingCost: _,
            buySellTaxes: H,
            buyLoanPrincipal: d,
            buyTotalCost: Y
        }
    ...
    i = r(l), o.on("edit.runtime", n), o.on("editend.runtime", n),
        function() {
            function e() {
                var e = "relative" === S.style("position"),
                    a = w.node().clientWidth + (e ? 14 : -O.node().clientWidth),
                    r = a - v.left - v.right;
                r !== b && (b = r, S.style("margin-left", e ? "-10px" : null).style("margin-right", e ? null : -x.right + "px"), T.attr("width", a + x.right), I.attr("transform", "translate(" + (b + v.right) + ",0)").call(C.tickSize(-b - v.left - v.right)), q.attr("transform", "translate(" + (b + 8) + ",0)").call(P), D.each(function(e) {
                    e.scale.range([0, b]), t.select(this).call(e.axis)
                }), _.attr("x1", -8).attr("x2", b + 8), H.attr("transform", function(t) {
            ...
            function n(e, a) {
                (null == a || "buyPrice" === a) && (R.domain([0, l.buyPrice / (5e3 / F)]), C.ticks(F / 40), P.ticks(F / 40, "$s"), I.call(C), q.call(P)), B.text(s(l.rentPerMonth)), $.attr("d", i(B.node().getComputedTextLength() + 10)), Y.style("bottom", function(t) {
            ...
            var b, v = {
                ...
                R = t.scale.linear(),
                C = t.svg.axis().scale(R).orient("right").tickFormat(""),
                P = t.svg.axis().scale(R).orient("right").tickPadding(4),
                k = {
                    year: {
                        parse: function(t) {
            ...
            R.range([0, -F]);
            var S = w.insert("div", "*").attr("class", "g-parameter-chart"),
                T = S.append("svg").attr("height", function(t) {
                    return t.chartHeight + v.top + v.bottom
                }),
                A = T.append("g").attr("transform", function(t) {
            ...
            var I = A.append("g").attr("class", "g-axis g-axis--y g-axis--y-inner"),
                q = A.append("g").attr("class", "g-axis g-axis--y g-axis--y-outer"),
                D = A.append("g").attr("class", "g-axis g-axis--x").attr("transform", "translate(0," + (R(0) + 10) + ")").each(function(e) {
                    e.axis = t.svg.axis().orient("bottom").scale(e.scale).ticks(5, e.scaleFormat).tickSize(5).tickPadding(5)
                });
            D.append("line").attr("class", "g-baseline-halo"), D.append("line").attr("class", "g-baseline");
            var _ = D.selectAll(".g-baseline,.g-baseline-halo").attr("y1", -7).attr("y2", -7);
            ...
            var L = A.append("rect").style("fill", "none").style("pointer-events", "all").style("cursor", "ew-resize").attr("ontouchstart" in document ? {
                ...
                G = 120,
                Y = S.append("svg").attr("class", "g-parameter-tip").attr("width", G).attr("height", 25).style("margin-left", v.left - G / 2 + "px").style("margin-bottom", v.bottom + 4 + "px"),
                z = Y.append("g").attr("transform", "translate(" + G / 2 + ",20)"),
                $ = z.append("path"),
                B = z.append("text").attr("y", -6),
                O = w.append("div").attr("class", "g-parameter-input");
            O.append("input").property("value", function(t) {
                return t.format(t.get())
            }).property("disabled", "ontouchstart" in document).on("keydown", g).on("change", m);
            var f = O.append("svg").attr("class", "g-parameter-reset").attr("width", 19).attr("height", 19).on("click", f).on("touchstart", f).append("g").attr("transform", "translate(9.5,9.5)");
        ...
        function() {
            function e() {
                d.classed("g-negative", l.rentPerMonth <= 0).classed("g-positive", l.rentPerMonth > 0), h.text(function(t) {
                    return t(l, i, s)
                })
            }
            ...
            function a() {
                var t = u.getBoundingClientRect(),
                    ...
                    i = d.node().getBoundingClientRect(),
                    o = i.bottom <= a.height;
                console.log(i.bottom, a.bottom), r != e && p.classed("g-fixed", r = e), n != o && p.classed("g-fixed-bottom", n = o)
            }
            var r = !1,
                n = !1,
                u = document.querySelector("#g-fixed-container"),
                c = document.querySelector("#g-fixed"),
                p = t.select("#g-fixed"),
                d = t.select(".interactive-graphic"),
                h = t.selectAll(".g-value").datum(function() {
