! function() {
    var secure = 0 === window.location.protocol.indexOf("https"),
        config = {
            kitId: secure ? "snb1ove" : "zam5nzz",
            host: secure ? "//use.typekit.net/" : "//typeface.nytimes.com/",
            scriptTimeout: 3e3
        },
    ...
    s.parentNode.insertBefore(tk, s)
        ...
        return engine = {
            ...
            isUserInAnyVariant: function(query) {
                var test = this.findTest(query);
                return test ? test.isUserInAnyVariant() : !1
            },
        ...
        var classLength, device, orientationType, viewportClass, eventQueue = {},
            ...
            checkSize = function() {
                var i, len, breakpoint, integer, width = getWidth(),
                    newArray = [];
                for (i = 0, len = breakpoints.length; len > i && (breakpoint = breakpoints[i], width >= breakpoint.val || breakpoint.key === breakpoints[0].key); ++i) newArray.push(prefix + "-" + breakpoint.key);
            ...
            processFlags = function(flags) {
                ...
                for (featureFlags = flags || [], i = 0, len = featureFlags.length; len > i; i += 1) featureClasses += " flag-" + featureFlags[i] + "-on";
                addClass(featureClasses)
            },
            getFlags = function() {
                return featureFlags
            },
            writeLogo = function(size, path, sectionName, mastheadOverrides, pageTheme, targetId) {
                var html, w, h, logoPath, isTransparent, displayColor, $logoLink = document.getElementById(targetId || ""),
                    isSmallViewport = getViewportInteger() < 110,
                    isGlobal = document.cookie.indexOf("NYT-Edition=edition|GLOBAL") >= 0;
                mastheadOverrides = mastheadOverrides || "", isTransparent = mastheadOverrides.indexOf("masthead-theme-transparent") > -1 || mastheadOverrides.indexOf("masthead-theme-transparent-ffffff") > -1, displayColor = mastheadOverrides.indexOf("masthead-theme-transparent-ffffff") > -1 || "black" === pageTheme ? "white-" : "", "large" === size ? (w = isGlobal ? "450" : "370", h = isGlobal ? "44" : "50") : "magazine" === sectionName ? (w = isSmallViewport ? "184" : "260", h = isSmallViewport ? "22" : "23") : "t-magazine" === sectionName ? (w = isSmallViewport ? "184" : "250", h = isSmallViewport ? "22" : "30") : isGlobal ? (w = isTransparent ? "240" : "236", h = isTransparent ? "30" : "22") : (w = isTransparent ? "163" : "185", h = isTransparent ? "30" : "26"), logoPath = path + (isGlobal ? "international-logo-" : "nyt-logo-") + displayColor + w + "x" + h, "magazine" === sectionName && (logoPath = path + "magazine-logo-" + displayColor + w + "x" + h), "t-magazine" === sectionName && (logoPath = path + "t-magazine-logo-" + displayColor + w + "x" + h), isGlobal && "mobile" === device && (w = "184", h = "26"), html = '<svg class="nyt-logo" width="' + w + '" height="' + h + '" role="img" aria-label="The New York Times"><image width="' + w + '" height="' + h + '" xlink:href="' + logoPath + '.svg" src="' + logoPath + '.png" alt="The New York Times" border="0" /></svg><img src="' + logoPath + '.png" class="nyt-logo-print" alt="The New York Times" />', $logoLink ? $logoLink.innerHTML += html : document.write(html)
            },
            addAbTestClass = function() {
                var i, regex = new RegExp("has-ab-test-[\\w]*-[\\d]", "g"),
                    classNamesToAdd = [],
                    classNames = window.NYTABTEST.engine.getClassNamesForVariants();
                for (classNames = classNames.length > 0 ? classNames.split(" ") : [], i = 0; i < classNames.length; i += 1) classNamesToAdd.push("has-ab-test-" + classNames[i]);
            ...
            addViewportMetaFromUrlParam = function() {
                ...
                match && match[1] && (width = +match[1]), 1050 >= width && (viewportTag = document.createElement("meta"), viewportTag.setAttribute("name", "viewport"), viewportTag.setAttribute("content", "width=" + width + ", initial-scale=1, maximum-scale=1"), document.getElementsByTagName("head")[0].appendChild(viewportTag), checkSize())
            },
            ...
            init = function() {
                return 0 === getClasses().length && setClasses(""), detectDevice(), detectMoz(), detectEmbedPosition(), checkSize(), processFlags(), addAbTestClass(), window.onresize = checkSize, "mobile" === device && (orientationChange(), window.onorientationchange = orientationChange), {
                    on: on,
                    device: device,
                    getWidth: getWidth,
                    getHeight: getHeight,
